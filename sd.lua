...
local library = {
    ...
    folders = {
        main = "gamesneeze",
        assets = "gamesneeze/Images",
        configs = "gamesneeze/Configs"
    },
    shared = {
        initialized = false,
        fps = 0,
...
-- Theme Variables
local theme = {
    accent = Color3.fromRGB(105, 241, 255),
    lightcontrast = Color3.fromRGB(34, 39, 41),
    darkcontrast = Color3.fromRGB(20, 24, 26),
...
local themes = {
    ...
    ["gamesense"] = {
        ...
        textdark = Color3.fromRGB(89, 89, 89),
        textborder = Color3.fromRGB(0, 0, 0),
        cursoroutline = Color3.fromRGB(15, 15, 15),
        font = 2,
        textsize = 13,
    },

    ["fatality"] = {
        accent = Color3.fromRGB(255, 10, 50),
        lightcontrast = Color3.fromRGB(36, 40, 42),
        darkcontrast = Color3.fromRGB(22, 25, 27),
        outline = Color3.fromRGB(0, 0, 0),
    ...
    ["neverlose"] = {
        ...
        lightcontrast = Color3.fromRGB(38, 43, 45),
        darkcontrast = Color3.fromRGB(23, 27, 30),
        outline = Color3.fromRGB(0, 0, 0),
        inline = Color3.fromRGB(52, 52, 52),
        textcolor = Color3.fromRGB(255, 255, 255),
        textdark = Color3.fromRGB(180, 180, 180),
        textborder = Color3.fromRGB(0, 0, 0),
        cursoroutline = Color3.fromRGB(25, 25, 25),
        font = 2,
        textsize = 13,
    },
    ...
    ["nekocheat"] = {
        ...
        lightcontrast = Color3.fromRGB(42, 47, 49),
        darkcontrast = Color3.fromRGB(28, 32, 35),
        outline = Color3.fromRGB(0, 0, 0),
        inline = Color3.fromRGB(65, 65, 65),
        textcolor = Color3.fromRGB(255, 255, 255),
        textdark = Color3.fromRGB(170, 170, 170),
        textborder = Color3.fromRGB(0, 0, 0),
        cursoroutline = Color3.fromRGB(30, 30, 30),
        font = 2,
        textsize = 13,
    }
}
...
local activeThemeName = "default"         -- default theme name
local activeTheme = themes[activeThemeName] -- reference its table
...
function applyTheme(themeName)
    ...
    activeTheme     = cloneTheme(theme) -- make a fresh copy every time you load

    for object, properties in pairs(library.colors) do
        for prop, themeKey in pairs(properties) do
            if activeTheme[themeKey] then
                object[prop] = activeTheme[themeKey]
            end
        end
    end
end
...
do
    ...
    function library:Loader(info)
        ...
        theme.accent = accent
        --
        local window = {pages = {}, loader = true, isVisible = false, pageammount = pageammount, callback = callback, wminfo = "gamesneeze - ping : %s - fps : %u", currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil, textbox = nil}}
        --
    ...
    function library:New(info)
		local info = info or {}
        local name = info.name or info.Name or info.title or info.Title or "gamesneeze"
        local size = info.size or info.Size or Vector2.new(510,630)
        local accent = info.accent or info.Accent or info.color or info.Color or theme.accent
        local callback = info.callback or info.Callback or info.callBack or info.CallBack or function() end
        ...
        --
        local window = {pages = {}, loader = style == 2, init = false, pageammount = pageammount, isVisible = false, callback = callback, uibind = Enum.KeyCode.Z, wminfo = "danielbot - ping : %s - fps : %u", currentPage = nil, fading = false, dragging = false, drag = Vector2.new(0,0), currentContent = {frame = nil, dropdown = nil, multibox = nil, colorpicker = nil, keybind = nil, textbox = nil}}
        --
        local main_frame = utility:Create("Frame", {Vector2.new(0,0)}, {
            Size = utility:Size(0, size.X, 0, size.Y),
            Position = utility:Position(0.5, -(size.X/2) ,0.5, -(size.Y/2)),
            Color = activeTheme.outline
        });window["main_frame"] = main_frame
        --
        library.colors[main_frame] = {
            Color = "outline"
        ...
        local inner_frame = utility:Create("Frame", {Vector2.new(1,1), frame_inline}, {
            Size = utility:Size(1, -2, 1, -2, frame_inline),
            Position = utility:Position(0, 1, 0, 1, frame_inline),
            Color = activeTheme.lightcontrast
        })
        --
        library.colors[inner_frame] = {
            Color = "lightcontrast"
        ...
        local title = utility:Create("TextLabel", {Vector2.new(4,2), inner_frame}, {
            Text = name,
            Size = activeTheme.textsize,
            Font = activeTheme.font,
            Color = activeTheme.textcolor,
            OutlineColor = activeTheme.textborder,
            Position = utility:Position(0, 4, 0, 2, inner_frame)
        })
        --
        library.colors[title] = {
        ...
        local back_frame = utility:Create("Frame", {Vector2.new(1,1), inner_frame_inline2}, {
            Size = utility:Size(1, -2, 1, -2, inner_frame_inline2),
            Position = utility:Position(0, 1, 0, 1, inner_frame_inline2),
            Color = activeTheme.darkcontrast
        });window["back_frame"] = back_frame
        --
        library.colors[back_frame] = {
            Color = "darkcontrast"
        ...
        function window:Watermark(info)
            ...
            --
            local text_bounds = utility:GetTextBounds(watermark_name, activeTheme.textsize, activeTheme.font)
            --
    ...
    function library:Page(info)
        ...
        --
        local textbounds = utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font)
        --
        ...
        function page:Show()
            if window.currentPage then
                ...
                window.currentPage.page_button_color.Color = activeTheme.darkcontrast
                window.currentPage.page_button_title.Color = activeTheme.textdark
            ...
            page_button_color.Color = activeTheme.lightcontrast
            page_button_title.Color = activeTheme.textcolor
            page.open = true
            --
            library.colors[page_button_color] = {
                Color = "lightcontrast"
    ...
    function sections:Toggle(info)
        ...
        local toggle_frame = utility:Create("Frame", {Vector2.new(1,1), toggle_inline}, {
            ...
            Color = toggle.current == true and activeTheme.accent or activeTheme.lightcontrast,
            Visible = page.open
        }, section.visibleContent)
        --
        ...
        --
        local toggle_title = utility:Create("TextLabel", {Vector2.new(23,toggle.axis + (15/2) - (utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = activeTheme.textsize,
            Font = activeTheme.font,
            Color = activeTheme.textcolor,
            OutlineColor = activeTheme.textborder,
            Position = utility:Position(0, 23, 0, toggle.axis + (15/2) - (utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font).Y/2), section.section_frame),
            Visible = page.open
        }, section.visibleContent)
        --
        ...
        function toggle:Set(bool)
            if typeof(bool) == "boolean" then
                toggle.current = bool
                toggle_frame.Color = toggle.current == true and activeTheme.accent or activeTheme.lightcontrast
                --
                library.colors[toggle_frame] = {
                    Color = toggle.current == true and "accent" or "lightcontrast"
                }
        ...
        function toggle:Keybind(info)
            ...
            function keybind:Reset()
                for i,v in pairs(keybind.modemenu.buttons) do
                    v.Color = v.Text == keybind.mode and activeTheme.accent or activeTheme.textcolor
                    --
                    library.colors[v] = {
            ...
            library.began[#library.began + 1] = function(Input)
                ...
                if keybind.selecting and window.isVisible then
                    ...
                    if done then
                        ...
                        keybind_frame.Color = activeTheme.lightcontrast
                        --
                        library.colors[keybind_frame] = {
                            Color = "lightcontrast"
                        }
                ...
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and window.isVisible and keybind_outline.Visible then
                    if utility:MouseOverDrawing({section.section_frame.Position.X + (section.section_frame.Size.X - (40+4+2)), section.section_frame.Position.Y + keybind.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + keybind.axis + 17}) and not window:IsOverContent() and not keybind.selecting then
                        ...
                        keybind_frame.Color = activeTheme.darkcontrast
                        --
                        library.colors[keybind_frame] = {
                            Color = "darkcontrast"
                        }
                    ...
                    if utility:MouseOverDrawing({section.section_frame.Position.X  + (section.section_frame.Size.X - (40+4+2)), section.section_frame.Position.Y + keybind.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + keybind.axis + 17}) and not window:IsOverContent() and not keybind.selecting then
                        ...
                        for i,v in pairs({"Always", "Toggle", "On Hold", "Off Hold"}) do
                            local button_title = utility:Create("TextLabel", {Vector2.new(modemenu_frame.Size.X/2,15 * (i-1)), modemenu_frame}, {
                                ...
                                Size = activeTheme.textsize,
                                Font = activeTheme.font,
                                Color = v == keybind.mode and activeTheme.accent or activeTheme.textcolor,
                                Center = true,
                                OutlineColor = activeTheme.textborder,
    ...
    function sections:TextBox(info)
        ...
        local textbox_value = utility:Create("TextLabel", {Vector2.new(textbox_frame.Size.X/2,0), textbox_frame}, {
            ...
            Size = activeTheme.textsize,
            Font = activeTheme.font,
            Color = textbox.current == "" and (placeholder and activeTheme.textdark) or activeTheme.textcolor,
            OutlineColor = activeTheme.textborder,
            Center = true,
        ...
        function textbox:Set(state, first)
            ...
            textbox_value.Color = textbox.current == "" and (placeholder and activeTheme.textdark) or activeTheme.textcolor
            --
            library.colors[textbox_value] = {
                OutlineColor = "textborder",
                Color = textbox.current == "" and (placeholder and "textdark") or "textcolor"
            ...
            if Input.UserInputType == Enum.UserInputType.MouseButton1 and textbox_outline.Visible and window.isVisible then
                if reactive and utility:MouseOverDrawing({section.section_frame.Position.X, section.section_frame.Position.Y + textbox.axis, section.section_frame.Position.X + section.section_frame.Size.X, section.section_frame.Position.Y + textbox.axis + 20}) and not window:IsOverContent() then
                    ...
                    if not (window.currentContent.textbox and window.currentContent.textbox.Name == identifier) then
                        window:CloseContent()
                        --
                        textbox_value.Color = activeTheme.accent
                        --
                        library.colors[textbox_value] = {
                            OutlineColor = "textborder",
                            Color = "accent"
        ...
        function dropdown:Update()
            if dropdown.open and dropdown.holder.inline then
                for i,v in pairs(dropdown.holder.buttons) do
                    ...
                    --
                    v[1].Text = value
                    v[1].Color = value == tostring(dropdown.current) and activeTheme.accent or activeTheme.textcolor
                        ...
                        for Index = 1, (max and max or #dropdown.options) do
                            ...
                            if Value then
                                ...
                                local dropdown_value = utility:Create("TextLabel", {Vector2.new(Value == tostring(dropdown.current) and 8 or 6,2), dropdown_value_frame}, {
                                    ...
                                    Size = activeTheme.textsize,
                                    Font = activeTheme.font,
                                    Color = Value == tostring(dropdown.current) and activeTheme.accent or activeTheme.textcolor,
                                    OutlineColor = activeTheme.textborder,
    ...
    function sections:Keybind(info)
        ...
        --
        local keybind_title = utility:Create("TextLabel", {Vector2.new(4,keybind.axis + (17/2) - (utility:GetTextBounds(name, activeTheme.textsize, theme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = activeTheme.textsize,
            Font = activeTheme.font,
            Color = activeTheme.textcolor,
            OutlineColor = activeTheme.textborder,
            Position = utility:Position(0, 4, 0, keybind.axis + (17/2) - (utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font).Y/2), section.section_frame),
            Visible = page.open
        }, section.visibleContent)
        --
        library.colors[keybind_title] = {
    ...
    function sections:Colorpicker(info)
        ...
        --
        local colorpicker_title = utility:Create("TextLabel", {Vector2.new(4,colorpicker.axis + (15/2) - (utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font).Y/2)), section.section_frame}, {
            Text = name,
            Size = activeTheme.textsize,
            Font = activeTheme.font,
            Color = activeTheme.textcolor,
            OutlineColor = activeTheme.textborder,
            Position = utility:Position(0, 4, 0, colorpicker.axis + (15/2) - (utility:GetTextBounds(name, activeTheme.textsize, activeTheme.font).Y/2), section.section_frame),
            Visible = page.open
        }, section.visibleContent)
        --
        library.colors[colorpicker_title] = {
